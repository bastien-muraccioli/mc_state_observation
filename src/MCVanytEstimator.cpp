...
void MCVanytEstimator::runTiltEstimator(const mc_control::MCController & ctl, const mc_rbdyn::Robot & updatedRobot)
{
  ...
  */
  ...
  // If the following variable is not set, it means that no contact was detected and thus we are in the case 2 as
  // defined above.
  ...
  so::kine::Kinematics anchorFrameKine = odometryManager_.getAnchorFrameKinematics(ctl, imuSensor_);

  so::kine::Kinematics imuAnchorKine = updatedWorldImuKine_.getInverse() * anchorFrameKine;

  std::cout << std::endl << "anchorFrameKine" << std::endl << anchorFrameKine << std::endl;
  std::cout << std::endl << "imuAnchorKine" << std::endl << imuAnchorKine << std::endl;

  x1Debug_ = -imu.angularVelocity().cross(updatedImuAnchorKine_.position()) - imuAnchorKine.linVel();
  ...
  auto onMaintainedContactObserver = [this](const odometry::LoContactWithSensor & maintainedContact)
  {
    const so::kine::Kinematics & worldContactRefKine = maintainedContact.worldRefKine_;
    ...
    const so::kine::Kinematics & contactFbKineOdometryRobot = maintainedContact.contactFbKine_;
  ...
  auto runParams = odometry::LeggedOdometryManager::RunParameters(poseW_).attitude(R_0_fb_).onMaintainedContact(
