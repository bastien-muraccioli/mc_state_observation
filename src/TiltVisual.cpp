...
TiltVisual::TiltVisual(const std::string & type, double dt, bool asBackup, const std::string & observerName)
...
void TiltVisual::configure(const mc_control::MCController & ctl, const mc_rtc::Configuration & config)
{
...
void TiltVisual::reset(const mc_control::MCController & ctl)
{
  const auto & robot = ctl.robot(robot_);
...
void TiltVisual::updateAnchorFrameOdometry(const mc_control::MCController & ctl)
{
...
void TiltVisual::updateAnchorFrameNoOdometry(const mc_control::MCController & ctl, const mc_rbdyn::Robot & updatedRobot)
{
  const auto & robot = ctl.robot(robot_);
...
void TiltVisual::runTiltEstimator(const mc_control::MCController & ctl, const mc_rbdyn::Robot & updatedRobot)
{
  /*
  ...
  */
  ...
  if(odometryManager_.odometryType_ != measurements::OdometryType::None)
  {
    ...
    odometryManager_.correctContacts(R_0_fb_);
...
void TiltVisual::update(mc_rbdyn::Robot & robot)
{
  robot.posW(poseW_);
  robot.velW(velW_);
}

const so::kine::Kinematics TiltVisual::backupFb(boost::circular_buffer<so::kine::Kinematics> * koBackupFbKinematics)
{
...
so::kine::Kinematics TiltVisual::applyLastTransformation(const so::kine::Kinematics & previousKine)
{
...
void TiltVisual::setOdometryType(OdometryType newOdometryType)
{
  odometryManager_.setOdometryType(newOdometryType);
}

void TiltVisual::addToLogger(const mc_control::MCController & ctl,
...
void TiltVisual::removeFromLogger(mc_rtc::Logger & logger, const std::string & category)
{
...
void TiltVisual::addToGUI(const mc_control::MCController &,
...
EXPORT_OBSERVER_MODULE("TiltVisual", mc_state_observation::TiltVisual)
